...
export default class Shelly1PMGen4Driver extends ShellyLocalDriver {
  async onPairMatchDevice(deviceInfo: ShellyGetDeviceInfoResponse): Promise<boolean> {
    return deviceInfo.id.toLowerCase().startsWith('shelly1pmg4');
  }
  ...
  async assembleHomeyDevices(
    ...
    const device: ShellyLocalListDeviceProperties = {
      ...
      icon: '../../../assets/drivers/shelly1pmg4/icon.svg',
      store: {
