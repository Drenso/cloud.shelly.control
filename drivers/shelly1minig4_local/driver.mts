...
export default class Shelly1MiniGen4Driver extends ShellyLocalDriver {
  async onPairMatchDevice(deviceInfo: ShellyGetDeviceInfoResponse): Promise<boolean> {
    return deviceInfo.id.toLowerCase().startsWith('shelly1minig4');
  }
  ...
  async assembleHomeyDevices(
    ...
    const device: ShellyLocalListDeviceProperties = {
      ...
      icon: '../../../assets/drivers/shelly1minig4/icon.svg',
      store: {
