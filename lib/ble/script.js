...
function forward(result) {
  ...
  Shelly.emitEvent('homey_ble_forward', result);
}
...
function init() {
  if (BLE.Scanner === undefined) {
    // Zigbee firmware or Matter use can disable Bluetooth
    throw new Error('Error: Bluetooth is not available with this device configuration');
  }
